...
function seed() {
  ...
  const fromToday = (off: number) => {
    // relative to today (keeps "Upcoming" populated regardless of date)
  ...
  events = [
    ...
    mk(fixed(12), 'Design Conference', 'purple', {
      end: fixed(13),
      description: 'Two-day design & UX conference downtown.'
    }),
    ...
    mk(fromToday(9), 'Product Webinar', 'purple', {
      allDay: false,
      time: '16:00',
      description: 'Live demo of the new dashboard.'
    }),
...
let fTime: HTMLInputElement,
  fColor: HTMLSelectElement,
  fAllDay: HTMLInputElement,
  fDesc: HTMLTextAreaElement
...
function renderMonth() {
  ...
  for (let i = 0; i < 42; i++) {
    ...
    html += `<div data-date="${dISO}"
      ...
      <div class="space-y-1">${shown.map(pill).join('')}${
        extra > 0
...
function renderFilters() {
  filtersBox.innerHTML = (Object.keys(COLORS) as ColorKey[])
    .map(
      (key) => `
...
function saveEvent() {
  ...
  if (!t) {
    fTitle.focus()
    fTitle.classList.add('ring-2', 'ring-red-500')
    return
  }
...
export default function initCalendar() {
  ...
  document.querySelectorAll<HTMLElement>('.cal-view-btn').forEach((b) =>
    b.addEventListener('click', () => {
      view = b.dataset.view as 'month' | 'list'
      render()
    })
  )
  ...
  // Create button
  document
    .getElementById('cal-create')
    ?.addEventListener('click', () => openModal('add', { start: todayISO() }))
  ...
  fAllDay.addEventListener('change', () => {
    fTime.disabled = fAllDay.checked
    if (fAllDay.checked) fTime.value = ''
  })
  ...
  document
    .querySelectorAll('[data-cal-close]')
  ...
  document.querySelectorAll<HTMLElement>('.cal-ext-event').forEach((chip) => {
    chip.addEventListener('dragstart', (e) => {
      e.dataTransfer?.setData(
        'text/plain',
        JSON.stringify({ title: chip.dataset.title, color: chip.dataset.color })
      )
  ...
  grid.addEventListener('dragover', (e) => {
    ...
    if (cell) {
      e.preventDefault()
      cell.classList.add('ring-2', 'ring-inset', 'ring-blue-400')
    }
  })
  grid.addEventListener('dragleave', (e) => {
    ;(e.target as HTMLElement)
      .closest('[data-date]')
      ?.classList.remove('ring-2', 'ring-inset', 'ring-blue-400')
  })
